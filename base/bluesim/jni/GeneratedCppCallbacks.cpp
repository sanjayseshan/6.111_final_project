...
extern const uint32_t ifcNames_MemcpyIndicationH2S = IfcNames_MemcpyIndicationH2S;
extern const uint32_t ifcNames_MemcpyRequestS2H = IfcNames_MemcpyRequestS2H;
...
/************** Start of MemcpyRequestWrapper CPP ***********/
#include "MemcpyRequest.h"
int MemcpyRequestdisconnect_cb (struct PortalInternal *p) {
    (static_cast<MemcpyRequestWrapper *>(p->parent))->disconnect();
    return 0;
};
int MemcpyRequeststartCopy_cb (  struct PortalInternal *p, const uint32_t wrPointer, const uint32_t rdPointer, const uint32_t numWords, const uint32_t burstLen, const uint32_t iterCnt ) {
    (static_cast<MemcpyRequestWrapper *>(p->parent))->startCopy ( wrPointer, rdPointer, numWords, burstLen, iterCnt);
    return 0;
};
MemcpyRequestCb MemcpyRequest_cbTable = {
    MemcpyRequestdisconnect_cb,
    MemcpyRequeststartCopy_cb,
};

/************** Start of MemcpyIndicationWrapper CPP ***********/
#include "MemcpyIndication.h"
int MemcpyIndicationdisconnect_cb (struct PortalInternal *p) {
    (static_cast<MemcpyIndicationWrapper *>(p->parent))->disconnect();
    return 0;
};
int MemcpyIndicationstarted_cb (  struct PortalInternal *p ) {
    (static_cast<MemcpyIndicationWrapper *>(p->parent))->started ( );
    return 0;
};
int MemcpyIndicationdone_cb (  struct PortalInternal *p ) {
    (static_cast<MemcpyIndicationWrapper *>(p->parent))->done ( );
    return 0;
};
MemcpyIndicationCb MemcpyIndication_cbTable = {
    MemcpyIndicationdisconnect_cb,
    MemcpyIndicationstarted_cb,
    MemcpyIndicationdone_cb,
};
